...
- **真实 issue 评测输入**：benchmark case 支持 `repo_url`、固定 `base_ref`、自定义 `test_command`、`issue_text`、`issue_url`、ground truth PR/commit。
- **统一 diff 补丁**：真实模型优先输出 unified diff，并通过 `git apply` 应用；fallback 仍支持 snippet patch，方便无 Key 测试。
- **Issue 关键词定位**：运行时会从任务和 issue 文本提取关键词，调用代码搜索并把搜索结果写入 trace。
...
真实开源 issue case 可以参考：

```text
benchmark/cases/real_issues.example.json
```
